  ...
  return (
    <div className="flex flex-col md:flex-row h-full w-full gap-4 p-5 overflow-hidden font-sans text-slate-100">
      ...
      <div className="w-full md:w-80 lg:w-96 shrink-0 h-full">
        {/* Pairing Form Card */}
        <div className="bg-slate-900 border border-slate-800 rounded-lg p-5 flex flex-col gap-4 shadow-sm h-full">
          ...
          <form onSubmit={handlePairSubmit} className="flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              ...
              <input
                ...
                className="bg-slate-950 border border-slate-800 text-sm rounded-md px-3 py-2.5 text-slate-200 focus:outline-none focus:border-indigo-500 font-mono transition-colors"
              />
            </div>
            ...
            <button
              ...
              className="mt-2 w-full flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-800 disabled:text-slate-500 text-white font-medium py-2.5 rounded-md text-xs transition cursor-pointer"
            >
      ...
      <div className="flex-1 bg-slate-900 border border-slate-800 rounded-lg p-5 flex flex-col gap-4 overflow-hidden shadow-sm h-full">
        {/* Panel Header - min-h-10 for baseline symmetry, allows growth */}
        <div className="flex items-center justify-between border-b border-slate-800 pb-3 shrink-0 min-h-10">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-800 hover:bg-slate-700 disabled:opacity-50 text-slate-300 rounded-md text-xs font-medium transition cursor-pointer"
          >
            <RefreshCw
          ...
          {devices.length === 0 ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-3">
              {devices.map((device) => (
                <div
                  key={device.edc_id}
                  className="bg-slate-950 border border-slate-800/80 rounded-lg p-3 flex items-center justify-between gap-3 hover:border-slate-700 transition shadow-sm"
                >
                  <div className="flex flex-col overflow-hidden">
                    ...
                    <span
                      className="font-mono text-sm font-bold text-slate-200 truncate"
                      title={device.edc_id}
                    >
                      {device.edc_id}
                    </span>
                  ...
                  <button
                    ...
                    className="shrink-0 flex items-center gap-1.5 px-2.5 py-1.5 bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 border border-rose-500/20 rounded-md text-[11px] font-medium transition-colors cursor-pointer focus:ring-2 focus:ring-rose-500/50 outline-none"
